...
teammates:
  ...
  - name: challenger
    ...
    model: sonnet
    phase: [1, 2, 3]        # 各フェーズの意思決定ゲート直前に召集
...
tasks:
  ...
  - id: TD1
    name: 悪魔の代弁者レビュー — リサーチ仮説への挑戦
    assignee: challenger
    description: >
      T01〜T03 の調査結果と TL0 の法的スクリーニングに基づき、
      チームが暗黙的に持っている市場・ユーザー・競合前提を徹底的に批判する。
      「このリサーチが間違っているとしたら何が原因か」「採用率・市場規模の根拠は楽観的すぎないか」
      「競合の参入障壁を過小評価していないか」を中心に CRITICAL / HIGH / MEDIUM で分類して報告する。
      最低3つの失敗シナリオと、T04 で回答が必要な重要問いを列挙すること。
    output: docs/phase1-devils-advocate.md
  ...
  - id: T04
    ...
    assignee: leader
    description: >
      TL0〜T03 統合 + TD1（悪魔の代弁者レポート）を踏まえた統合 →
      差別化マトリクス、Value Proposition、MVP機能最終確認、リスク評価。
      TD1 の CRITICAL 批判への回答（反論 or 設計変更）を成果物に明記すること。
      未回答のまま残した批判はリスクとして明示的に記録すること。
    ...
    blocked_by: [TL0, T01, T02, T03, TD1]
```

### Wave 1
```
Wave 1-A（並列）: TL0 + T01 + T02 + T03
Wave 1-B（逐次）: TD1  ← 悪魔の代弁者がリサーチ結果に反論
Wave 1-C（逐次）: T04  ← TD1 の批判への回答を含めて統合

⚠️ 悪魔の代弁者ゲート（Wave 1-B → 1-C）:
  TD1 の CRITICAL 批判 → T04 で必ず回答（反論 or 設計変更）すること
  未回答の CRITICAL が残る場合 → Phase 2 進行前に Leader が明示的にリスク受容判断を下す
...
tasks:
  ...
  - id: TD2
    name: 悪魔の代弁者レビュー — アーキテクチャ・戦略への挑戦
    assignee: challenger
    description: >
      T05（技術設計）・T06（UI/UX）・T07（マーケ戦略）の初期案に対して徹底的な批判を行う。
      「このアーキテクチャのスケーラビリティ・セキュリティ上の盲点は何か」
      「このマーケティング戦略はなぜ機能しないか」「UI/UXの前提はユーザー実態と乖離していないか」
      を中心に CRITICAL / HIGH / MEDIUM で分類して報告する。
      TL1（法的レビュー）と合わせて TL1 完了後に実施すること。
      最低3つの失敗シナリオと TL1 の法的批判と合わせた未回答重要問いを列挙すること。
    output: docs/phase2-devils-advocate.md
    blocked_by: [T05, T06, T07, TL1]

  - id: T08
    name: CEO 意思決定 — アーキテクチャ確定
    assignee: leader
    description: >
      T05〜T07 + TL1 + TD2（悪魔の代弁者レポート）統合 →
      技術スタック、MVP優先順位、戦略 確定。
      TD2 の CRITICAL 批判への回答（反論 or 設計変更）を成果物に明記すること。
...
Wave 2-B（並列）: TL1 + TD2  ← 法的レビューと悪魔の代弁者を並列実施
                              （TD2 は TL1 完了後に開始 — blocked_by: TL1）
Wave 2-C（逐次）: T08

⚠️ 悪魔の代弁者ゲート（Wave 2-B → 2-C）:
  TD2 の CRITICAL 批判 → T08 で必ず回答（反論 or 設計変更）すること
  未回答の CRITICAL が残る場合 → Leader が明示的にリスク受容判断を下す
...
tasks:
  ...
  - id: TD3
    name: 悪魔の代弁者レビュー — MVP仕様・ビジネスモデルへの最終挑戦
    assignee: challenger
    description: >
      T09（ペルソナ）・T10（機能要件）・T11（ビジネスモデル）に対して最終的な徹底批判を行う。
      「このペルソナ定義はユーザー実態から乖離していないか」
      「このビジネスモデルは競合に模倣された瞬間に崩壊しないか」
      「MVP機能の優先順位付けは本当に正しいか。見落とされている必須機能はないか」
      「収益化タイムラインは現実的か」を中心に批判する。
      これまでの TD1・TD2 で指摘した未解決の CRITICAL 批判が解消されているかも検証すること。
      TL2 完了後に開始し、法的リスクと合わせた総合的リスクプロファイルを提示する。
    output: docs/phase3-devils-advocate.md
    blocked_by: [T09, T10, T11, TL2]

  - id: T12
    name: CEO 意思決定 — MVP スコープ確定
    assignee: leader
    description: >
      T09〜T11 + TL2 + TD3（悪魔の代弁者最終レポート）統合 → MVP スコープ確定。
      TD3 の CRITICAL 批判への回答を最終決定に反映すること。
      TD1・TD2 から引き継がれた未解決リスクの最終的な受容・軽減判断を下すこと。
...
Wave 3-B（並列）: TL2 + TD3  ← 法的最終レビューと悪魔の代弁者最終批判を並列実施
                              （TD3 は TL2 完了後に開始 — blocked_by: TL2）
Wave 3-C（逐次）: T12

⚠️ 悪魔の代弁者ゲート（Wave 3-B → 3-C）:
  TD3 の CRITICAL 批判 → T12 で必ず回答（反論 or 設計変更）すること
  TD1・TD2 から引き継がれた未解決 CRITICAL → T12 で最終的なリスク受容判断を下す
  未回答のまま Phase 4 へ進む場合 → logs/decision-timeline.md に受容判断の根拠を記録
...
Phase 1: リサーチ
  Wave 1-A ─── TL0 + T01 + T02 + T03 ──（並列）
                 │
  Wave 1-B ─── TD1 ──────────────────────（逐次: 悪魔の代弁者がリサーチに反論）
                 │
              ⚠️ 悪魔の代弁者ゲート（TD1）
              ⚠️ 法的ゲート（TL0）
                 │
  Wave 1-C ─── T04 ──────────────────────（逐次: 批判への回答を含めて統合）

Phase 2: 技術設計
  Wave 2-A ─── T05 + T06 + T07 ──────────（並列、T04 完了後）
  Wave 2-B ─── TL1 → TD2 ────────────────（逐次: 法的レビュー後に悪魔の代弁者）
                 │
              ⚠️ 悪魔の代弁者ゲート（TD2）
              ⚠️ 法的ゲート（TL1）
                 │
...
Phase 3: プロダクト定義
  ...
  Wave 3-B ─── TL2 → TD3 ────────────────（逐次: 法的最終レビュー後に悪魔の代弁者）
                 │
              ⚠️ 悪魔の代弁者ゲート（TD3）＋累積リスク最終判断
              ⚠️ 法的ゲート（TL2）
                 │
  Wave 3-C ─── T12 ──────────────────────（逐次）
...
| Phase 1 | 悪魔の代弁者レポート（リサーチ批判） | `docs/phase1-devils-advocate.md` |
...
| Phase 2 | 悪魔の代弁者レポート（設計批判） | `docs/phase2-devils-advocate.md` |
...
| Phase 3 | 悪魔の代弁者レポート（MVP最終批判） | `docs/phase3-devils-advocate.md` |
